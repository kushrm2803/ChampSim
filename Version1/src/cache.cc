...
/*
    ...
    rest half are hot
    rest half are cold
    below 64*3 are very cold
*/

int global_set;
int global_cache_type;
int global_eviction_or_victim;
/*
  first we will call check_hit.
  then update global_set according to check_h.
  then call get_set and return global_set.
*/
vector<vector<int>> redirect(2048); // each hot set will redirect to three 3 cold sets
int access_count = 0; // count access count and updating 
...
int CACHE::check_hit(PACKET *packet)
{
    ...
    if(global_cache_type == IS_LLC && match_way == -1 && redirect[set].size()>0){
      for(auto set1 : redirect[set]){
        for(uint32_t way = 0; way<NUM_WAY; way++){
