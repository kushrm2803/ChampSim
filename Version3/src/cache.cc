...
// declarations
vector<int>access_freq(LLC_SET,0);  // stores how many times a set is accessed
vector<int>hotColdSet(LLC_SET,0);   // stores indexes of sets according to their hotness

int numOfHotSets = 84; // number of very hot sets
int sorting_cycle = 20000; // after this many cycles we will sort the sets
int numOfRedirectSetsperHotSet = 3; // number of sets that will redirect to one cold sets

unordered_set<int> very_hot; // all hot sets are stored in the set
int global_set; // used for returning the set index number
int global_cache_type;  // global_cache_type = cache_type
int global_eviction_or_victim;  // 1 means eviction/victim  and 0 means read
vector<vector<int>> redirect(LLC_SET); // each hot set will redirect to three cold sets
vector<unordered_set<int>> redirect_read(LLC_SET); // this will also redirect but it will used for reading not evicting or victim
long long access_count = 0; // count access count, evicting & victiming
/*
  How are we optimizing :-
  first we will call check_hit.
  then update global_set according to check_hit.
  then call get_set and return global_set which will get updated in check_hit.
*/

void CACHE::handle_fill()
{
    global_cache_type = cache_type;
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        // find victim
        if(cache_type == IS_LLC) global_eviction_or_victim = 1;
        uint32_t set = get_set(MSHR.entry[mshr_index].address), way;
            if(cache_type==IS_LLC)access_freq[(uint32_t) (MSHR.entry[mshr_index].address & ((1 << lg2(NUM_SET)) - 1))]++;
        if (cache_type == IS_LLC) {
            // if(cnt<100){cout<<set<<endl;cnt++;}
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        // if(cnt<100){cout<<set<<endl;cnt++;}
        if(cache_type==IS_LLC)access_freq[(uint32_t) (WQ.entry[index].address & ((1 << lg2(NUM_SET)) - 1))]++;
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
      ...
      if (read_cpu == NUM_CPUS)
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
            ...
            if(cache_type==IS_LLC){
              access_freq[(uint32_t) (RQ.entry[index].address & ((1 << lg2(NUM_SET)) - 1))]++;
            }
            if (way >= 0) { // read hit
      ...
      if (prefetch_cpu == NUM_CPUS)
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
            ...
            uint32_t set = get_set(PQ.entry[index].address);
             if(cache_type==IS_LLC)access_freq[(uint32_t) (PQ.entry[index].address & ((1 << lg2(NUM_SET)) - 1))]++;
...
void CACHE::classifySets(){
  vector<pair<int,int>>setWithAcess;
  for(int i = 0;i<2048;i++)setWithAcess.push_back(make_pair(access_freq[i],i));
  sort(setWithAcess.begin(),setWithAcess.end(), greater<pair<int, int>> ());
  //Hot sets are at top
  // Cold sets are at bottom
  hotColdSet.clear();
  for(uint32_t i=0;i<setWithAcess.size();i++){
    hotColdSet.push_back(setWithAcess[i].second);
  }

  very_hot.clear();
  for(int i=0;i<2048;i++) redirect[i].clear();

  int j = LLC_SET - 1;  // index of last element in hotColdSet

  for(int i=0;i<numOfHotSets;i++){
    
    very_hot.insert(hotColdSet[i]); // all very hot sets are stored in the set

    int ind = hotColdSet[i];//index of hot sets
    // hotColdSet[j] is index of cold set
    // redirecting sets
    for(int k=0;k<numOfRedirectSetsperHotSet;k++){
      redirect[ind].push_back(hotColdSet[j]);
      redirect_read[ind].insert(hotColdSet[j--]);
    }
  }
}
...
void CACHE::operate()
{
    ...
    // if(access_count == sorting_cycle*1000){
    //   access_count = 1;
    //   hotColdSet.clear();
    //   very_hot.clear();
    //   for(int i=0;i<redirect.size();i++) redirect[i].clear();
    //   for(int i=0;i<redirect_read.size();i++) redirect_read[i].clear();
    //   for(int i=0;i<access_freq.size();i++) access_freq[i] = 0;
    // }
    if(access_count%sorting_cycle==0){
      classifySets(); //classifysets would take more time if executed each time
    }
...
{
    if(global_cache_type == IS_LLC){

      if(global_eviction_or_victim == 1){
        global_eviction_or_victim = 0;
        uint32_t set = (uint32_t) (address & ((1 << lg2(NUM_SET)) - 1)); 
        vector<uint32_t> temp;
        temp.push_back(set);
        for(auto it: redirect[set]){temp.push_back(it);}
        global_set = temp[rand()%temp.size()];
      }
      return global_set;
    }
...
// no use; don't know why given :(
// to find way instruction goes to check_hit :)
...
int CACHE::check_hit(PACKET *packet)
{
    ...
    global_set =  (uint32_t) (packet->address & ((1 << lg2(NUM_SET)) - 1)); 
    uint32_t set = global_set;
    int match_way = -1;
    ...
    //above code is as it is
    // below we are checking there can be a hit in the redirect_set
    // if hit, we will update global_set and match_way
    //here the code changes from redirect_set to redirect_set_read
    if(global_cache_type == IS_LLC && match_way == -1 && redirect_read[set].size()>0){
      vector<int> delset;
      bool delflag = true;  // the set will be deleted.
      for(auto set1 : redirect_read[set]){
        for(uint32_t way = 0; way<NUM_WAY; way++){
          if( (uint32_t) (block[set1][way].tag & ((1 << lg2(NUM_SET)) - 1)) != set1) delflag = false;
          if (block[set1][way].valid && (block[set1][way].tag == packet->address)) {
            ...
            DP ( if (warmup_complete[packet->cpu]) {
            cout << "[" << NAME << "] " << __func__ << " instr_id: " << packet->instr_id << " type: " << +packet->type << hex << " addr: " << packet->address;
            cout << " full_addr: " << packet->full_addr << " tag: " << block[set1][way].tag << " data: " << block[set1][way].data << dec;
            cout << " set: " << set << " way: " << way << " lru: " << block[set1][way].lru;
            cout << " event: " << packet->event_cycle << " cycle: " << current_core_cycle[cpu] << endl; });
        ...
        // if the set is not very_hot and there is not a hit then there we cannot read from it
        // in future so we will remove it from read set;
        if(delflag && match_way == -1 && very_hot.find(set) == very_hot.end()){
          delset.push_back(set1);
        }
      }
      for(auto it: delset) redirect_read[set].erase(it);
    } 
    if( global_cache_type == IS_LLC && match_way == -1 && redirect_read[set].size()>0){
      vector<uint32_t> temp;
      temp.push_back(set);
...
// using namespace std;
// uint64_t l2pf_access = 0;
// vector<int>access_freq(LLC_SET,0);
// vector<int>hotncoldSets(LLC_SET,0);
// vector<int>coldSets(LLC_SET/2,0);
// vector<int>hotSets(LLC_SET/2,0);
// uint64_t cnt=0;
// int clk=0; // so that sets are not classified at every call
...
//         // find victim
//         uint32_t set = get_set(MSHR.entry[mshr_index].address), way;
//             if(cache_type==IS_LLC)access_freq[set]++;
//         if (cache_type == IS_LLC) {
//             // if(cnt<100){cout<<set<<endl;cnt++;}
//             way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type);
//         }
...
// void CACHE::classifySets(){
//   uint64_t partitionSet=LLC_SET/2;
//   vector<pair<int,int>>setWithAcess;
//   for(int i = 0;i<2048;i++)setWithAcess.push_back(make_pair(access_freq[i],i));
//   sort(setWithAcess.begin(),setWithAcess.end());
//   // Hotsets are the sets from the last and cold sets are the sets from the start of the vector
//   hotncoldSets.clear();
//   for(uint32_t i=0;i<LLC_SET;i++){
//     hotncoldSets.push_back(setWithAcess[i].second);
//   }
//   // for(auto i: hotncoldSets)cout<<i<<endl;
//   // coldSets.clear();
//   // hotSets.clear();
//   // // coldset is stored in sorted i.e from starting very cold then it changes toward cold
//   // for(uint32_t i=0;i<partitionSet;i++){
//   //   coldSets.push_back(setWithAcess[i].second);
//   // }
//   // // hotset is stored as last set would be very hot and first would be least hot
//   // for(uint32_t i=partitionSet;i<LLC_SET;i++){
//   //   hotSets.push_back(setWithAcess[i].second);
//   // }
//   // cout<<"__________________HOT__________________"<<endl;
//   // for(auto i:hotSets){
//   //   cout<<"SET: "<<i<<endl;
//   // }
//   // cout<<"__________________COLD__________________"<<endl;
//   // for(auto i:coldSets){
//   //   cout<<"SET: "<<i<<endl;
//   // }
//   // cout<<hotSets.size()<<' '<<coldSets.size()<<endl;
// }

// void CACHE::operate()
// {
//   // cout<<"ACCESS_______________________________________________"<<endl;
//   // if(cnt<1000){
//   //   for(auto i:access_freq){cout<<i.first<<" "<<i.second<<endl;cnt++;}
//   // }
//     clk++;
//     handle_fill();
//     handle_writeback();
//     reads_available_this_cycle = MAX_READ;
//     handle_read();
//     if(clk%10000==0)classifySets(); //classifysets would take more time if executed each time
//      if (PQ.occupancy && (reads_available_this_cycle > 0))
//         handle_prefetch();
// }
