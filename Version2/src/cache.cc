...
vector<int>hotColdSet(LLC_SET,0);
/*
    top 64 are very hot 
    rest half are hot
    rest half are cold
    below 64 are very cold
*/
int numOfHotSets = 64; // number of very hot sets
int sorting_cycle = 10000; // after this many cycles we will sort the sets
int numOfRedirectSetsperHotSet = 2; // number of sets that will redirect to one cold sets

set<int> very_hot; // all hot sets are stored in the set
int global_set; // used for returning the set number
...
/*
  ...
  then update global_set according to check_h.
  then call get_set and return global_set.
*/
vector<vector<int>> redirect(LLC_SET); // each hot set will redirect to one cold sets
vector<set<int>> redirect_read(LLC_SET); // this will also redirect but it will used for reading not evicting
long long access_count = 0; // count access count and updating 
...
int CACHE::check_hit(PACKET *packet)
{
    ...
    if(global_cache_type == IS_LLC && match_way == -1 && redirect_read[set].size()>0){
      ...
      for(auto set1 : redirect_read[set]){
        ...
        if(match_way == -1 && very_hot.find(set) == very_hot.end()){
